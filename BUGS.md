...
## BUG-045
**Status:** Found, NOT fixed — deliberately out of scope for this task (see below), needs its own session
**Severity:** Critical (the "Enable sharing" button fails outright, with an alert, for any intersection or area-study project — likely the most common project types)
**Found in:** v3.46.0-alpha.1 (Item 5) — `enableSharing()`/`pushSharedUpdate()` in `src/share.js`, calling Firestore's `setDoc()`
**Description:** Firestore documents don't support arrays-of-arrays ("nested arrays") as a value type. An intersection project's serialized `periods[].vData` is `{in: [[...]], out: [[...]]}` (array of per-interval arrays) and `pedData` is `[[[...]]]` (crosswalk × interval × direction) — both nested arrays. `setDoc()` rejects the whole write with "Function setDoc() called with invalid data. Nested arrays are not supported." Confirmed live: clicking "Enable sharing" on a real intersection project (any project with vehicle or pedestrian data) fails with this exact error, shown to the user as "Could not enable sharing. Check your connection and try again." — a misleading message, since the actual cause has nothing to do with connectivity. An area study embeds full intersection snapshots (same `vData`/`pedData` shape) and would fail the same way. Parking (`grid: {slotIdx: {zoneId: count}}`, no arrays) was confirmed live to write successfully through a real `setDoc()` call. Trip Gen's shape (`entries[].days[].parsed.intervals[].inbound`/`outbound` — one-dimensional arrays, not nested) was not exercised against a real `setDoc()` call in this session (this task's Trip Gen verification used hand-fed data through the `viewerMode` render path directly, bypassing Firestore, per the same approach the v3.46.0-alpha.1 DEVLOG entry used before Anonymous auth was enabled) — worth a quick real-`enableSharing()` check in whatever session picks up BUG-045, since it's plausible but not yet confirmed to be unaffected.
**Root cause:** `serializeCurrentProject()`'s intersection/area shape was designed for `localStorage`/JSON export, which has no such restriction — nobody hit this until a real Firestore write was attempted with real vehicle/pedestrian data. The scoping/research pass and initial verification for Item 5 (v3.46.0-alpha.1) used Anonymous auth that wasn't yet enabled in the Firebase Console, so the real `setDoc()` call was never actually reached during that work — DEVLOG.md's v3.46.0-alpha.1 entry explicitly flags this gap ("What's blocked, not worked around"). This task's live testing was the first time a real `setDoc()` call for an intersection project actually executed.
**Why not fixed here:** this task's brief explicitly scoped out "the underlying data model, the Firestore push/pull mechanism" — fixing this means either flattening/restructuring `vData`/`pedData` before the Firestore write (and correspondingly on read) or changing the serialized shape itself, both of which are data-model/push-mechanism changes, not a viewer-rendering change. Documenting it here rather than silently leaving it undiscovered.
**Suggested fix direction (not implemented):** `enableSharing()`/`pushSharedUpdate()` could flatten nested arrays into a Firestore-safe shape (e.g. a flat array with an explicit stride, or a plain object keyed by index) immediately before `setDoc()`, and `fetchSharedProject()` would need the inverse un-flatten before handing data to `loadProject()`. Needs its own session — touches the exact code this task was told not to touch.
**Verified live:** reproduced on a real intersection project against the user's own provisioned Firebase project (not a mock) — `setDoc()` genuinely rejects it. Parking confirmed to work correctly end-to-end through a real share link (real `setDoc()`, real second-tab fetch) in the same session (see DEVLOG.md).
